...
dfm %<>% mutate_at(vars(emo:pro), list(~ (. * 2) + 5.5))
```
...
# WARNING! because control group has only 3 possible measurments so far,
...
# the other say is to throw the waves out and work with MM/YYYY times
...
map2(
  ...
  ~ ggpredict(.x, c("wave", "group", "gender")) %>%
    ...
    scale_x_discrete(limits = wave_ind_labs[1:3]) +
    labs(
