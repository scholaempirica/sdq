---
title: "SDQ"
subtitle: "průběžné výsledky po 7. vlně"
output:
...
rnorm(1e6, mean = 166.2, sd = 6.4) %>%
  as_tibble() %>%
  ggplot(aes(value, group  =1, label = "asdf")) +
...
```{r aad, fig.cap=fig_counter("Srovnání trajektorií kontrolních školek")}
...
```{r aaasd, fig.cap=fig_counter("Srovnání trajektorií intervenčních školek")}
...
# change_by_wave %>%
#   filter(wave_contr == "wave2") %>% 
#   ggplot(aes(y = school, x = estimate)) +
#   geom_pointrange(aes(
#     xmin = (-1.96 * std.error) + estimate,
#     xmax = (1.96 * std.error) + estimate
#   )) + facet_wrap(~ dim)
