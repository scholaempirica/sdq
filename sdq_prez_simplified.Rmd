...
- ze  7. a neúplnné 8. vlny máme záznamy od `r min_date` do `r max_date`
...
- ve zbytku prezentace **pracujeme s ind. vlnami** (*x*-tým měřením/hodnocením dítěte)
  - k dispozici máme 3 vlny s jak intervenční, tak i kontrolní skupinou
...
- následující grafy ukazují počty dětí, které získaly daný počet bodů v dané dimenzi SDQ
   - celkem 3 časové úseky (vlny), červená je skupina kontrolní, modrá intervenční
...
dfm %>% filter(wave_ind == 1) %>% 
  ...
  labs(x = "body v SDQ", y = "počet dětí") +
  theme_schola(multiplot = TRUE, legend.position = c(.825, 0.250), axis.title = element_text())
```
...
mod <-
  tibble(
    ...
    model = map(dim, ~ lmer(
      ...
      dfmod %>% filter(wave %in% c(1:3)) # only 3 ind waves!
    ))
  )
```
...
interv_in_age %>% transmute(dimenze = dim %>% fct_recode(!!!abbrevs), "v 2. vlně" = age_compens_w2, "v 3. vlně" = age_compens_w3) %>% kable
...
- zobrazený je vývoj oproti tzv. referenční školce (sestava nezařazených pozorování)
...
plot_school_effect <- function(.data, dim = "emo", nrow = 1, ncol = NULL, facet = FALSE) {
  .data %>%
    filter(dim == {{ dim }}) %>%
...
dim_labs %>%
  names() %>%
  walk(~ print(change_by_wave_con %>% plot_school_effect(dim = .x, ncol = NULL, nrow = 1) + theme(legend.title = element_blank(), legend.position = "bottom", legend.text = element_text(size = 9),
    legend.key.size = unit(.1, "mm"))))
...
dim_labs %>%
  names() %>%
  walk(~ print(change_by_wave %>% plot_school_effect(dim = .x, nrow = 2) + theme(
    legend.title = element_blank(), legend.position = "bottom", legend.text = element_text(size = 9),
    legend.key.size = unit(.1, "mm")
  )))
