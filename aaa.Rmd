...
```{r, fig.cap="graf test"}
min_date <- dfm %>% filter(wave == 7) %>% pull(fill_in_date) %>% lubridate::ymd() %>% min(na.rm = T) %>% format("%d. %m. %Y")
max_date <- dfm %>% filter(wave == 7) %>% pull(fill_in_date) %>% lubridate::ymd() %>% max(na.rm = T) %>% format("%d. %m. %Y")
...
- doposud `r child_n` dětí proělo alespoň jedním měřením
...
read_csv(here("data-processed/cleaning_impact_wave.csv")) %>%
  ...
  kable(
    col.names = c(
      ...
      "poč. ztrac. obs.",
      "pod. ztrac. obs."
    ), caption = "Dopad validace dat - globální vlny"
  )
...
```{r cars, echo=FALSE, fig.cap="ajajajajsdjf oiasjdfoasdfasdfčšřčřščřš!!!", fig.height=4, message=FALSE, dev='svg'}
...
read_csv(here("data-processed/cleaning_impact_wave_ind.csv")) %>%
  ...
  kable(
    col.names = c(
      ...
      "poč. ztrac. obs.",
      "pod. ztrac. obs."
    ), caption = "Dopad validace dat - individuální vlny", format = 'html'
  )
```
...
library(pander)
read_csv(here("data-processed/cleaning_impact_wave_ind.csv")) %>%
  mutate(`% lost` = scales::percent(`% lost`/100, suffix = " %", accuracy = .1, decimal.mark = ",")) %>% pander(style= "rmarkdown", caption = "akakadfjasodi foashdfasdf", )
```
