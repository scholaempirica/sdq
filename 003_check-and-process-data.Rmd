...
```{r}
dfm %>% filter_at(vars(contains("invalid")), all_vars(. == FALSE)) %>% count(wave) %>%
...
# in R 4.0 theres is no need to renam??
# rename and turn list into tibble
thetas_tbl <-
  map_dfr(
    thetas,
    ~ .x %>% select(rowid, emo:pro) #%>% rename(
      # "emo" = F1,
      # "con" = F2,
      # "hyp" = F3,
      # "pee" = F4,
...
time_dependency <- function(df, dim, method = "pearson") {
  ...
  ) %>% select(-id_pupil) %>% corrr::correlate(method = method) %>% corrr::shave(upper = FALSE) %>% corrr::fashion()
}
